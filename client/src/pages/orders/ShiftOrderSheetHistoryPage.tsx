...
/**
 * The Shift Order Sheet archive.
 *
 * This used to share a route with the current Sheet, revealed by a `historyOpen`
 * flag. One flag decided which of two unrelated screens rendered, so the filter
 * rail was mounted over the current Sheet — which has nothing to filter — and
 * neither view could be linked to or bookmarked. The route is the flag now, and
 * the filters live on the only screen that uses them.
 */
const ShiftOrderSheetHistoryPage = () => {
  useDocumentTitle('Lịch sử phiếu order ca');
  const { user, role } = useAuth();
  const assignedAreaId = user?.publicData.area_id ?? '';
  const currentSheetPath = getWorkspacePath(role, SHIFT_ORDER_SHEET_PATH);
  ...
  if (selectedHistoryId) {
    if (selectedHistoryQuery.isPending) {
      ...
      content = !sheet.area || !sheet.work_shift ? (
        ...
        <section className="space-y-4">
          {/* Selecting a Sheet keeps the reader on this route, so the way back to
              the filtered list is a state reset, not history navigation. */}
          <button type="button" className={SecondaryButton} onClick={() => setSelectedHistoryId(null)}>
            ← Quay lại danh sách
          </button>
          <ShiftOrderSheetWorkspace
            ...
            currentSheetPath={assignedAreaId ? currentSheetPath : undefined}
          />
        </section>
      );
    }
    ...
    content = (
      <section className="min-w-0 space-y-5">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          {assignedAreaId && (
            <Link to={currentSheetPath} className={SecondaryButton}>
              Phiếu hiện tại
            </Link>
          )}
        </header>
...
export default ShiftOrderSheetHistoryPage;
