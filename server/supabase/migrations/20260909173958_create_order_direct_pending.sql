-- Phase 2: create a complete PENDING Order in one database transaction.
-- New application Orders use only this RPC.
...
revoke all on function public.create_pending_order_with_items(
  text, uuid, uuid, uuid, text, jsonb, uuid, timestamptz
)
...
comment on function public.create_pending_order_with_items(
  ...
  'Atomically validates positive stock, resolves the current Shift Sheet, and creates a PENDING Order with all OrderItems.';
